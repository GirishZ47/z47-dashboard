...
def render():
    ...
    if opening_soon:
        ...
        _warn("No opening-soon data from live source. Showing pipeline below.")
        fb = pd.DataFrame([
            {"Company": f["company"], "Sector": f["sector"], "Status": f["status"],
             "Price Band": f["price_band"], "Issue Size": f["issue_size"],
             "Expected Listing": f["expected_listing"]}
            for f in UPCOMING_FALLBACK if f["status"] in ("DRHP Filed", "Expected 2025", "Expected")
        ])
    ...
    pl = pd.DataFrame([
        {"Company": f["company"], "Sector": f["sector"], "Status": f["status"],
         "Price Band": f["price_band"], "Issue Size": f["issue_size"],
         "Expected": f["expected_listing"], "About": f["description"]}
        for f in UPCOMING_FALLBACK
    ])
    st.dataframe(pl, use_container_width=True, hide_index=True,
                 column_config={"About": st.column_config.TextColumn(width="large")})
