...
def get_z47_index_takeaway_v2(monday_key: str = "") -> str | None:
    ...
    prompt = (
        ...
        "DRHP filings, IPO news.\n\n"
        "Write exactly 6 sections with the following fixed headers and structures.\n\n"
        ...
        "  Sub-points: block deals, lock-in expiries, institutional flow dynamics. "
        "Q4 earnings cohort split. Non-obvious structural insight.\n\n"
        "SECTION 3 — Header: 'Macro ; <one-line verdict on how global pressures hit asymmetrically>'\n"
        "  Sub-points: oil/FX/rate impact, monsoon/El Nino risk, digital-adoption structural offset.\n\n"
        "SECTION 4 — Header: 'Top gainers'\n"
        "  Sub-points: 2 companies with **Company Name (+X.X%)** format, semicolon, "
        "cause-effect chain (why it moved, what the market is pricing).\n\n"
        "SECTION 5 — Header: 'Top laggards'\n"
        "  Sub-points: 2 companies with **Company Name (–X.X%)** format (use en-dash – for negative), "
        "semicolon, cause-effect chain (what the market is pricing, what could change it).\n\n"
        "SECTION 6 — Header: 'Net Read'\n"
        "  Sub-points: 2 lines. First: index structural read in one sentence. "
        "Second: the single near-term catalyst (earnings print, regulatory event, data point) "
        "that will determine the next move.\n\n"
        "QUALITY RULES:\n"
        "- No em-dashes (—) anywhere. Use a semicolon (;) to join label ; verdict.\n"
        "- Use en-dash (–) only for negative returns: –12.3%.\n"
        ...
        "- No MF portfolio lists. Reference MF category flows (e.g. 'domestic small-cap MF demand') "
        "not fund-by-fund lists.\n"
        "- No phrases that recur verbatim across sections (e.g. 'structural' may appear at most "
        "twice in the full output; find synonyms for the third instance).\n"
        "- No preamble, no section numbers. Output the 6 sections only."
    )
    return _ai_takeaway(system, prompt, max_tokens=1800)
