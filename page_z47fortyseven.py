...
# takeaway_constants are now consumed by takeaway_generator.py (standalone script).
# The app reads from auto_monthly_takeaway.json generated by GitHub Actions.
...
def _s_kissht_takeaway() -> None:
    ...
    body_html = _build_sections_html(sections)
    st.markdown(
...
def _render_header_bar(df: pd.DataFrame, usdinr: float,
    ...
    _auto_tk   = _load_auto_takeaway()
    tk_updated = _auto_tk.get("updated", "—") if _auto_tk else "—"
...
def _build_sections_html(sections: list) -> str:
    """
    Shared sections renderer used by both Monthly Takeaway and Kissht IPO Takeaway.
    main_bullet WITH sub_bullets: large orange bullet for header + small dots for content.
    main_bullet WITHOUT sub_bullets: small plain dot, tight spacing (sub-bullet style).
    section_title: orange small-caps label + small dots.
    """
    body_html = ""
    is_first = True
    for sec in sections:
        ...
        if stype == "section_title":
            ...
            # main_bullet WITH sub_bullets: large orange header + indented sub-bullet content
            mt = "0" if is_first else "22px"
            if " ; " in header:
            ...
            # main_bullet WITHOUT sub_bullets (e.g. Kissht full-sentence items):
            # render as sub-bullet style — small plain dot, tight spacing, regular weight
            mt = "0" if is_first else "10px"
            body_html += (
                f'<div style="position:relative;padding-left:18px;margin-top:{mt}">'
                f'<span style="position:absolute;left:0;top:6px;color:{_BLK};'
                f'font-size:8px;line-height:1">&#9679;</span>'
                f'<p style="margin:0;font-size:13.5px;line-height:1.65;'
                f'font-weight:400;color:{_BLK};{_F}">'
                f'{_process_bold(header)}</p>'
                f'</div>'
            )
...
def _s4_takeaway(override_tk=None) -> None:
    ...
    tk       = override_tk if override_tk is not None else {}
...
def _load_auto_takeaway():
    """Load auto-generated monthly takeaway from JSON file written by GitHub Actions."""
    import json
    _path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "auto_monthly_takeaway.json")
    try:
...
def _s4_monthly_takeaway(df, returns_1m, mcaps, name_map,
    ...
    """Monthly Takeaway — auto-published weekly by GitHub Actions; no draft/approval UI."""
    tk = _load_auto_takeaway()
    if tk is None:
        try:
            from takeaway_constants import HARDCODED_INDEX_TAKEAWAY as _HT
            tk = _HT
        except Exception:
            pass
    if tk:
        _s4_takeaway(override_tk=tk)
...
def _s10_footer(usdinr: float) -> None:
    ...
    _auto_tk = _load_auto_takeaway()
    updated  = _auto_tk.get("updated", "—") if _auto_tk else "—"
    st.markdown(
